...
<center><h2>Hello   <%= user.getFirstName() %>   <%= user.getLastName() %>     send a message from Here.</h2></center>
<h2> File Upload and Transfer:</h2>
<form action="uploadFile.jsp" method="post" enctype="multipart/form-data">
<div>
	<label for="duser"><b>Available users for transfer : </b></label> 
		<select id="Duser" name="Duser">
		<option value="Select">Select</option>
			<%
				List<User> users = AvailableUsers.getAvailableUsers(request.getParameter("username"));
				
				for (final User userk : users) {
			%>
				<option value="<%=userk.getUserId()%>"><%=userk.getUserName()%></option>
			<%
				}
			%>
		</select>
...
<br>
<b>Select a text file to Upload:</b>
<input type="file" name="file" size="50"/><br/><br/>
<label for="Wtext"><b>Enter the Subject Here : </b><input id="Wtext" type="text" name="wtext" placeholder="Hello" size="30" required></label><br><br/>
<input type="submit" value="Upload File" class="w3-btn w3-black"/>
</form>
</body>
</html>